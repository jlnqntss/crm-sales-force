const { mkdirSync, rmdirSync, readdirSync, renameSync, moveSync } = require("fs-extra");
...
async function main() {
  ...
  try {
    ...
    if( whatToExecute == 0 || whatToExecute == 1 ) {
        console.log("------------------------------------------------------------------ Convirtiendo proyecto SFDX...");
        ...
        console.log("------------------------------------------------------------------ Reconciliando perfiles...");
        execSync(`sfdx sfpowerkit:source:profile:reconcile -u ${username} -d "${tmpProfile}"`, {
          stdio: "inherit"
         });
        
        console.log("------------------------------------------------------------------ Renombrando perfiles y copiando a deploy...");
        ...
        await readdirSync(`${tmpProfile}`).forEach(file => {      
          ...
          renameSync(`${tmpProfile}/` + file, `${tmpProfile}/` + new_file, (err) => {
            if (err) throw err;
            console.log(error);
          });
          moveSync(`${tmpProfile}/` + new_file, `${deployDir}/profiles/` + new_file, {
            overwrite: true
          });
        });
