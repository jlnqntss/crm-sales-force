...
function main() {
  ...
  if (typeof tmpDir == "undefined")
    throw new Error(
      "Tienes que especificar un directorio de trabajo. Ej. C:/Temp/Packages/"
    );
  ...
  if (typeof packageName == "undefined")
    throw new Error(
      "Tienes que especificar un nombre de paquete. Ej. TarificaV1"
    );
  ...
  if (typeof accion === "undefined")
    throw new Error(
      "Tienes que especificar una accion 0:crear paquete / 1:hacer deploy"
    );
  ...
  if (accion == 0) {
    ...
    if (fs.existsSync(`${packageDir}/${packageName}.profiles`)) {
      let perfiles = fs
        .readFileSync(`${packageDir}/${packageName}.profiles`)
        .toString()
        .split(",");
      ...
      copySync(
        `${packageDir}/${packageName}.zip`,
        `${packageDir}/${packageName}.${username}.zip`,
        { overwrite: true }
      );
    }
  } else if (accion == 2) {
...
function sobreescribirPefiles(packageDir, packageName, perfiles, username) {
  ...
  execSync(
    `unzip -o -d "${packageDir}/tmppackage" "${packageDir}/${packageName}.zip`
  );

  console.log("\tCopiando perfiles...");
  perfiles.forEach((perfil) => {
    copySync(
      `${packageDir}/tmpprofiles/${perfil}.profile-meta.xml`,
      `${packageDir}/tmppackage/profiles/${perfil}.profile`,
      { overwrite: true }
    );
  });
  ...
  copySync(
    `${packageDir}/tmppackage/${packageName}.zip`,
    `${packageDir}/${packageName}.${username}.zip`,
    { overwrite: true }
  );
...
function crearPackage(gitCambios, packageDir, packageName) {
  ...
  moveSync(
    `${packageDir}/${packageName}/${packageName}.zip`,
    `${packageDir}/${packageName}.zip`,
    { overwrite: true }
  );
}
